...
esp_err_t sd_init(void) {
  ...
  host.slot =
      SPI2_HOST; // попробуй SPI2_HOST; если не работает — пробуй SPI3_HOST
  ...
  ESP_LOGI(TAG, "SD mounted (name=%s, capacity=%lluMB)", card->cid.name,
           (unsigned long long)(card->csd.capacity / (1024ULL * 1024ULL)));
  s_mounted = true;
  return ESP_OK;
}
...
esp_err_t sd_close(FILE *fp) {
  ...
  if (s_mounted) {
    ...
    // esp_vfs_fat_sdcard_unmount exists in some IDF versions; if not, call
    // esp_vfs_fat_sdcard_unmount alternative To be robust, we will call
    // esp_vfs_fat_sdcard_unmount if available. If not — ignore.
